class_name EntityRenderSync3D
extends RefCounted

## Equivalente 3D de EntityRenderSync (game/render/entity_render_sync.gd) —
## mismo patrón, un solo multimesh_set_buffer() por grupo por frame
## (exploracion-3d.md sección 2A: "el patrón no cambia, cambia el tamaño
## del paquete"), ahora en formato TRANSFORM_3D (12 floats por instancia —
## 3 filas de la matriz básica + origen, el mismo layout que arma
## Transform3D internamente) en vez de TRANSFORM_2D (8 floats).
##
## Mapeo de coordenadas fijo para todo el puente de render 3D
## (fase-3d-tarjetas-pantallas-v1.md sección 1, decisión de una sola línea,
## consistente en todo el puente): X sim → X 3D, Y sim → Z 3D, altura fija
## en `height`.
##
## Sin rotación por instancia todavía (mismo alcance que el placeholder de
## proyectiles de poc_3d_bench.gd — cápsulas sin orientar) — no lo pide
## esta tarjeta, queda anotado como pendiente de Arte/motor cuando haga
## falta apuntado real en 3D.
...
## descubierto por captura real (fase-3d-motor-log.md): sin este factor,
...
## Color por-instancia según type_id (mismo motivo que EntityRenderSync 2D
## set_type_colors() — acá para proyectiles, que no necesitan sprite/malla
## distinta por tipo, solo distinguirse a simple vista, así que no
## necesitan un MultiMesh separado por tipo como sí necesitan las torres
## vía TypedRenderGroup3D). El material asignado debe tener
