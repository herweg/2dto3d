...
## Overlay global de FPS (fase-3d-tarjetas-pantallas-v1.md, Tarjeta 5) —
## autoload-escena (mismo mecanismo que StressLaunchConfig: un autoload
## puede ser una escena completa, no solo un script) para que aparezca en
## todas las pantallas (MainMenu, Level3D, Level1, StressMenu,
## ConfigMenu...) sin duplicarlo a mano en cada una — un autoload vive
## fuera del árbol de la escena actual, sobrevive a change_scene_to_file().
##
