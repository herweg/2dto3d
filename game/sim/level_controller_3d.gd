...
## Puente de render 3D real (fase-3d-tarjetas-pantallas-v1.md, Tarjeta 1) —
## la pantalla de juego real en 3D, no la POC de investigación
## (poc_3d_bench.gd sigue existiendo tal cual, sin tocar, como referencia y
## como el camino de la Tarjeta 4 mientras esta pantalla no exponga sus
## propios presets). Misma simulación de siempre (game/sim/*, Vector2 puro,
## sin ninguna referencia a 3D — ver docs/exploracion-3d.md sección 1),
## solo cambia la capa de render: EntityRenderSync/TypedRenderGroup (2D,
## level_controller.gd) → EntityRenderSync3D/TypedRenderGroup3D/
## SharedSkeletonRenderGroup (game/render/*_3d.gd, este puente).
##
## Adaptado de level_controller.gd, no reescrito de cero — mismo esqueleto
## de estado (colocación/combate), mismo parseo de CLI de dos pasadas,
## mismo spawner/oro/vidas. Lo que se deja afuera a propósito en esta
## primera pasada (no lo pide esta tarjeta, level_controller.gd 2D lo sigue
## teniendo intacto, ver fase-3d-motor-log.md): VFX reales
## (fase3-vfx-exploracion-v1.md, placeholders de quad/partícula 2D — no
## traducidos a 3D todavía), stress-textures (reemplazo de textura 2D vía
## SpriteAtlas — sin sentido acá, el render 3D ya usa los assets reales por
## default), y los diagnósticos puntuales sprite-test/orientation-test/
...
var _proj_render: EntityRenderSync3D
var _tower_render: TypedRenderGroup3D
...
func _ready() -> void:
	...
	if ClassDB.class_exists("SimHotPath"):
		...
		push_error("[level3d] SimHotPath no está registrado — ¿falta compilar game/rust/? backend=native no va a andar.")
...
## Arma los 3 grupos de render (fase-3d-tarjetas-pantallas-v1.md sección 0):
## torres/proyectiles vía MultiMeshInstance3D (TypedRenderGroup3D/
## EntityRenderSync3D), enemigos vía esqueleto compartido
...
func _setup_render_groups() -> void:
	...
	if tower_scene:
		...
		tmp.free()
	if tower_mesh == null:
		push_error("[level3d] no se pudo extraer malla de " + TOWER_SCENE_PATH)
	_tower_render = TypedRenderGroup3D.new(TowerStore.TOWER_TYPE_STATS.size(), MAX_TOWERS, tower_mesh, tower_mat, TYPE_COLORS, 0.0, WORLD_SCALE)
	_tower_render.add_all_to(self)

	# Proyectil: primitiva placeholder (cápsula, sin sombreado — no hay
	# asset generado todavía, mismo criterio que poc_3d_bench.gd), un solo
	# MultiMesh con color por-instancia (no necesita malla propia por tipo,
	# a diferencia de torres — mismo motivo que ya separaba estas dos
	# técnicas en 2D, ver TypedRenderGroup3D).
	...
	_proj_render = EntityRenderSync3D.new(MAX_PROJ, proj_mesh, proj_mat, 20.0)
...
func _complete_round() -> void:
	...
	print("[level3d] ronda completa — objetivo: %d, muertes: %d, leaks: %d, oro ganado: %d" % [_level.wave_enemy_count, _lane_system.killed_count, _lane_system.leaked_count, GOLD_PER_ROUND])
...
func _lose_round() -> void:
	...
	if _test_button:
		_test_button.disabled = true
	print("[level3d] derrota — vidas agotadas, muertes: %d, leaks: %d" % [_lane_system.killed_count, _lane_system.leaked_count])
...
func _setup_stress_test() -> void:
	...
	var out_path := "res://benchmark_results/level3d_stress_%d.csv" % Time.get_unix_time_from_system()
	_stress_logger = BenchmarkLogger.new(out_path)
	print("[level3d] stress test: %d torres colocadas (objetivo %d), rampa a %d enemigos. Log: %s" % [placed, _stress_towers, _stress_enemies, out_path])
...
func _maybe_screenshot() -> void:
	...
	for t in _shot_times:
		if _elapsed >= t and not _shots_taken.has(t):
			...
			var path := "res://benchmark_results/level3d_screenshot_t%d.png" % int(t)
			img.save_png(path)
			print("[level3d] screenshot guardado: ", path)
